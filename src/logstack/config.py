...
class SecuritySettings(BaseSettings):
    ...
    rate_limit_rps: int = Field(
        default=2000, description="Rate limit requests per second per token"
    )
    rate_limit_burst: int = Field(
        default=10000, description="Rate limit burst capacity per token"
    )
    admin_token: str = Field(default="", description="Admin token for flush endpoint")
    api_keys: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Valid API keys with metadata"
    )

    @field_validator("api_keys", mode="before")
    def parse_api_keys(cls, v: Any) -> Dict[str, Dict[str, Any]]:
        """Parse API keys from JSON string or individual environment variables."""
        # Start with empty dict
        api_keys = {}

        # Method 1: Individual environment variables
        # Pattern: LOGSTACK_API_KEY_{NAME}_{FIELD}
        env_api_keys = cls._parse_individual_api_key_envs()
        if env_api_keys:
            api_keys.update(env_api_keys)

        # Method 2: JSON string from LOGSTACK_SECURITY_API_KEYS
        if isinstance(v, str):
            try:
                parsed = json.loads(v)
    ...
    @classmethod
    def _parse_individual_api_key_envs(cls) -> Dict[str, Dict[str, Any]]:
        """Parse individual API key environment variables."""

        # Pattern: LOGSTACK_API_KEY_{NAME}_{FIELD}
        pattern = re.compile(r"^LOGSTACK_API_KEY_([A-Z0-9_]+)_([A-Z_]+)$")

        # Group environment variables by API key name
        key_groups: Dict[str, Dict[str, Any]] = {}

        for env_name, env_value in os.environ.items():
            match = pattern.match(env_name)
            if match:
                key_name = match.group(1).lower()  # Convert to lowercase
                field_name = match.group(2).lower()  # Convert to lowercase
                ...
                if field_name == "active":
                    key_groups[key_name][field_name] = env_value.lower() in (
                        "true",
                        "1",
                        "yes",
                        "on",
                    ...
                    key_groups[key_name][field_name] = env_value

        # Convert to final API keys format
        api_keys = {}
        for key_name, fields in key_groups.items():
            if "token" in fields:
                token = fields.pop("token")
                api_keys[token] = fields

                # Set defaults for missing fields
                api_keys[token].setdefault("name", key_name)
                api_keys[token].setdefault("active", True)
...
class MaskingSettings(BaseSettings):
    ...
    tenant_overrides: Dict[str, List[str]] = Field(
        default={}, description="Per-tenant additional masking keys"
    )
...
class WALSettings(BaseSettings):
    ...
    # Storage paths
    wal_root_path: Path = Field(
        default=Path("./wal"), description="Root directory for WAL storage"
    )

    # Segment management
    segment_max_bytes: int = Field(
        default=134217728, description="Maximum segment size (128MB)"
    )
    rotation_time_active_minutes: int = Field(
        default=5, description="Active segment rotation time"
    )
    rotation_time_idle_hours: int = Field(
        default=1, description="Idle segment rotation time"
    )
    idle_threshold_minutes: int = Field(
        default=10, description="Minutes to consider segment idle"
    )
    min_rotation_bytes: int = Field(
        default=65536, description="Minimum size for time-based rotation (64KB)"
    )
    force_rotation_hours: int = Field(
        default=6, description="Force rotation after hours"
    )

    # Quotas
    token_wal_quota_bytes: int = Field(
        default=2147483648, description="Per-token WAL quota (2GB)"
    )
    token_wal_quota_age_hours: int = Field(
        default=24, description="Per-token WAL age quota"
    )
    disk_free_min_ratio: float = Field(
        default=0.20, description="Minimum disk free ratio"
    )

    # Manual flush
    flush_endpoint_enabled: bool = Field(
        default=True, description="Enable manual flush endpoint"
    )
...
class LokiSettings(BaseSettings):
    ...
    push_endpoint: str = Field(
        default="/loki/api/v1/push", description="Loki push endpoint"
    )
    timeout_seconds: int = Field(default=30, description="Request timeout")
    max_retries: int = Field(default=3, description="Maximum retry attempts")
    backoff_seconds: List[int] = Field(
        default=[5, 10, 20], description="Backoff intervals"
    )
    backoff_park_seconds: int = Field(
        default=60, description="Park time after max retries"
    )
    batch_max_entries: int = Field(
        default=1000, description="Maximum entries per batch"
    )
    batch_max_bytes: int = Field(
        default=1048576, description="Maximum batch size (1MB)"
    )
...
class ValidationSettings(BaseSettings):
    ...
    max_label_value_length: int = Field(
        default=64, description="Maximum label value length"
    )
...
def _has_individual_api_key_envs() -> bool:
    """Check if individual API key environment variables exist."""
    import os
    import re

    pattern = re.compile(r"^LOGSTACK_API_KEY_[A-Z0-9_]+_[A-Z_]+$")
    return any(pattern.match(env_name) for env_name in os.environ)
...
def _set_env_from_config(config_data: Dict[str, Any]) -> None:
    ...
    # Only set if no individual API key env vars exist and no JSON env var exists
    if (
        not _has_individual_api_key_envs()
        and "LOGSTACK_SECURITY_API_KEYS" not in os.environ
    ):
    ...
    if "LOGSTACK_MASKING_TENANT_OVERRIDES" not in os.environ:
        ...
        if tenant_overrides:
            ...
            os.environ["LOGSTACK_MASKING_TENANT_OVERRIDES"] = json.dumps(
                tenant_overrides
            )
